...
import (
	...
	"image"
	"image/color"
	_ "image/gif"  // 自动支持解析 GIF 输入
	_ "image/jpeg" // 自动支持解析 JPEG 输入
	"image/png"
	"io"
	"os"
)
...
func (s *Sprite) RlePcxDecode(rle []byte) (p []byte) {
	if len(rle) == 0 || s.rle <= 0 { return rle }
	p = make([]byte, int(s.Size[0])*int(s.Size[1]))
	i, j, k, w := 0, 0, 0, int(s.Size[0])
	for j < len(p) {
		n, d := 1, rle[i]
		if i < len(rle)-1 { i++ }
		if d >= 0xc0 { n = int(d & 0x3f); d = rle[i]; if i < len(rle)-1 { i++ } }
		for ; n > 0; n-- {
			if k < w && j < len(p) { p[j] = d; j++ }
			k++
			if k == s.rle { k = 0; n = 1 }
		}
	}
...
func (s *Sprite) Rle8Decode(rle []byte) (p []byte) {
	if len(rle) == 0 { return rle }
	p = make([]byte, int(s.Size[0])*int(s.Size[1]))
	i, j := 0, 0
	for j < len(p) {
		n, d := 1, rle[i]
		if i < len(rle)-1 { i++ }
		if d&0xc0 == 0x40 { n = int(d & 0x3f); d = rle[i]; if i < len(rle)-1 { i++ } }
		for ; n > 0; n-- { if j < len(p) { p[j] = d; j++ } }
	}
	return
}
...
func (s *Sprite) Rle5Decode(rle []byte) (p []byte) {
	...
	for j < len(p) {
		...
		dl := int(rle[i] & 0x7f)
		c := byte(0)
		if rle[i]>>7 != 0 { if i < len(rle)-1 { i++ }; c = rle[i] }
		if i < len(rle)-1 { i++ }
		for {
			if j < len(p) { p[j] = c; j++ }
			rl--
			if rl < 0 { dl--; if dl < 0 { break }; c = rle[i] & 0x1f; rl = int(rle[i] >> 5); if i < len(rle)-1 { i++ } }
		}
	}
	return
}
...
func (s *Sprite) Lz5Decode(rle []byte) (p []byte) {
	...
	for j < len(p) {
		...
		if i < len(rle)-1 { i++ }
		if ct&byte(1<<cts) != 0 {
			if d&0x3f == 0 { d = (d<<2 | int(rle[i])) + 1; if i < len(rle)-1 { i++ }; n = int(rle[i]) + 2; if i < len(rle)-1 { i++ } } else {
				rb |= byte(d & 0xc0 >> rbc); rbc += 2; n = int(d & 0x3f)
				if rbc < 8 { d = int(rle[i]) + 1; if i < len(rle)-1 { i++ } } else { d = int(rb) + 1; rb, rbc = 0, 0 }
			}
			for { if j < len(p) { p[j] = p[j-d]; j++ }; n--; if n < 0 { break } }
		} else {
			if d&0xe0 == 0 { n = int(rle[i]) + 8; if i < len(rle)-1 { i++ } } else { n = d >> 5; d &= 0x1f }
			for ; n > 0; n-- { if j < len(p) { p[j] = byte(d); j++ } }
		}
		cts++
		if cts >= 8 { ct, cts = rle[i], 0; if i < len(rle)-1 { i++ } }
	}
	return
}

func ReadPalette(f io.ReadSeeker, offset int64, size uint32, version2 bool) ([]uint32, error) {
	if _, err := f.Seek(offset, io.SeekStart); err != nil { return nil, err }
	rawCount := int(size) / 4
	depth := 1
	for depth < rawCount { depth *= 2 }
	...
	for i := 0; i < len(pal); i++ {
		var rgba [4]byte
		if i < rawCount {
			if err := binary.Read(f, binary.LittleEndian, rgba[:]); err != nil { return nil, err }
		}
		if !version2 {
			if i == 0 { rgba[3] = 0 } else { rgba[3] = 255 }
		}
...
func ExtractAllFrames(filename string) ([]SffFrameInfo, error) {
	...
	var h SffHeader
	var lofs, tofs uint32
	if err := h.Read(f, &lofs, &tofs); err != nil { return nil, err }
	...
	for i := 0; i < int(h.NumberOfSprites); i++ {
		...
		if h.Version[0] == 1 {
			read(&xofs); read(&size); read(&axis); read(&group); read(&number)
			if size > 0 {
			...
			read(&group); read(&number); read(&width); read(&height); read(&axis)
			frames = append(frames, SffFrameInfo{Group: int32(group), Item: int32(number), Width: int32(width), Height: int32(height), X: axis[0], Y: axis[1]})
			shofs += 28
		}
	}
...
func ExtractFrameAsPng(filename string, targetGroup int32, targetItem int32) ([]byte, error) {
	...
	for i := 0; i < int(h.NumberOfSprites); i++ {
		...
		if h.Version[0] == 1 {
			var xofs uint32
			var ps byte
			read(&xofs); read(&spr.DataSize); read(&spr.Offset); read(&spr.Group); read(&spr.Number); read(&spr.IndexOfPrevious); read(&ps)
			spr.DataOffset = uint32(shofs + 32)

			if spr.DataSize > 0 {
				if ps == 0 {
					blockEnd := int64(spr.DataOffset + spr.DataSize)
					if int64(xofs) > int64(spr.DataOffset) && int64(xofs) < blockEnd { blockEnd = int64(xofs) }
					scanStart := blockEnd - 769
					scanLimit := int64(spr.DataOffset + 128)
					palOffset := int64(-1)
					var b [1]byte
					for pos := scanStart; pos >= scanLimit; pos-- {
						f.Seek(pos, 0); f.Read(b[:])
						if b[0] == 0x0C { palOffset = pos; break }
					}
					if palOffset == -1 { palOffset = blockEnd - 769 }
			...
			read(&spr.Group); read(&spr.Number); read(&spr.Size); read(&spr.Offset); read(&spr.IndexOfPrevious)
			var format byte
			read(&format)
			spr.rle = -int(format)
			read(&spr.coldepth)
			var xofs uint32
			read(&xofs); read(&spr.DataSize)
			var tmp uint16
			read(&tmp); spr.palidx = int(tmp)
			read(&tmp)
			if tmp&1 == 0 { xofs += lofs } else { xofs += tofs }
			spr.DataOffset = xofs
			shofs += 28
		}
	...
	for _, spr := range sprites {
		if int32(spr.Group) == targetGroup && int32(spr.Number) == targetItem { target = spr; break }
	}
	if target == nil { return nil, fmt.Errorf("frame not found") }

	visited := make(map[uint16]bool)
	currIdx := uint16(0xFFFF)
	for i, s := range sprites { if s == target { currIdx = uint16(i); break } }

	for target.DataSize == 0 && target.IndexOfPrevious < uint16(len(sprites)) {
		if visited[currIdx] { return nil, fmt.Errorf("circular link detected") }
		...
		if h.Version[0] == 1 { target.Size = sourceSpr.Size }
	}
	if target.DataSize == 0 { return nil, fmt.Errorf("empty linked frame data") }
	...
	if h.Version[0] == 1 {
		...
		if encoding == 1 { target.rle = int(bpl) } else { target.rle = 0 }

		pcxDataStart := int64(target.DataOffset) + 128
		rleSize := target.PalOffset - pcxDataStart
		if rleSize <= 0 { rleSize = int64(target.DataSize) - 128 - 769 }
		...
		if format == 0 {
			...
			if 2 <= format && format <= 4 {
				...
				f.Read(px)
				switch format {
				case 2: target.PxlData = target.Rle8Decode(px)
				case 3: target.PxlData = target.Rle5Decode(px)
				case 4: target.PxlData = target.Lz5Decode(px)
				}
				...
				if err == nil {
					if format == 10 { 
						if pi, ok := img.(*image.Paletted); ok {
							...
							target.Size[0] = uint16(pi.Rect.Dx()); target.Size[1] = uint16(pi.Rect.Dy())
							target.IsRaw = false
						} else if gray, ok := img.(*image.Gray); ok {
							target.PxlData = gray.Pix
							target.Size[0] = uint16(gray.Rect.Dx()); target.Size[1] = uint16(gray.Rect.Dy())
							target.IsRaw = false
						}
					} else { 
						target.IsRaw = true
		...
		// 增强版：SFFv2 调色板安全回退与透明度保护
		if !target.IsRaw && len(target.Pal) == 0 {
			palidx := target.palidx
			found := false
			for loopLimit := 0; loopLimit < 10; loopLimit++ { // 防止死循环
	...
	if target.IsRaw && len(target.PxlData) > 0 {
		if target.PxlData[0] == 0x89 { return target.PxlData, nil }
...
// ==========================================
// 🛠️ 独家底层写入机制：真正实现 SFF 图像无损替换与格式自适应
// ==========================================
...
func ReplaceFrameWithPng(sffPath string, targetGroup int32, targetItem int32, imagePath string) error {
	...
	for i := 0; i < int(h.NumberOfSprites); i++ {
		...
		if int32(group) == targetGroup && int32(number) == targetItem {
			...
			f.Seek(0, io.SeekEnd)
			// 核心修复点：为适配原生 Format 11 的解包逻辑 (跳过 4 字节)，强制注入垫片
			dummyHeader := []byte{0, 0, 0, 0}
			f.Write(dummyHeader)
			_, err = f.Write(finalPngData)
			if err != nil { return fmt.Errorf("写入PNG数据失败: %v", err) }
			...
			// 强制清空 Flags，确保强制基于 lofs 定位，避免原生偏移算法崩溃
			...
			binary.Write(f, binary.LittleEndian, uint32(len(finalPngData)+4)) // 真实长度 + 4字节垫片
