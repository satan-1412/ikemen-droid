...
func ReadPalette(f io.ReadSeeker, offset int64, size uint32, version2 bool) ([]uint32, error) {
	...
	for i := 0; i < len(pal); i++ {
		...
		// 🔥 这里是修复 SFFv2 马赛克的核心：只有 v1 才强制替换 alpha
		if !version2 {
			if i == 0 {
				rgba[3] = 0
			} else {
				...
				if err == nil {
					if format == 10 {
						if pi, ok := img.(*image.Paletted); ok {
							...
							target.PxlData = rgba.Pix
							target.Size[0] = uint16(rgba.Rect.Dx())
							target.Size[1] = uint16(rgba.Rect.Dy())
							target.IsRaw = true
						}
					} else {
						target.IsRaw = true
		...
		if !target.IsRaw && len(target.Pal) == 0 {
			...
			for loopLimit := 0; loopLimit < 10; loopLimit++ {
				...
				if plSize == 0 {
					...
					target.Pal, _ = ReadPalette(f, int64(lofs+pofs), plSize, h.Version[0] >= 2)
					found = true
					break
				}
			}
