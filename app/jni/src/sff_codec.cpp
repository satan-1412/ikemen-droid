...
Java_org_libsdl_app_DesktopSystemView_decodeSffV2C(JNIEnv* env, jobject thiz, jbyteArray data, jint format, jint width, jint height, jint colorDepth, jbyteArray palette) {
    ...
    // 🚨核心修复：根据色彩深度动态分配内存！32位=4倍内存，完美容纳真彩色
    int bytes_per_pixel = (colorDepth == 32) ? 4 : ((colorDepth == 24) ? 3 : 1);
    int raw_len = width * height * bytes_per_pixel;
    uint8_t* pixels = (uint8_t*)malloc(raw_len);
    memset(pixels, 0, raw_len);

    // 双重安全解压
    if (format == 4) { // LZ5
        lz5_decompress_hardcore((uint8_t*)src_buf, src_len, pixels, raw_len);
    } else if (format == 2) { // RLE8
        int src_ptr = 4; 
        int dst_ptr = 0;
        while (dst_ptr < raw_len && src_ptr < src_len) {
            uint8_t byte = src_buf[src_ptr++];
            if ((byte & 0xC0) == 0x40) { 
                int count = byte & 0x3F;
                if (src_ptr >= src_len) break;
                uint8_t color = src_buf[src_ptr++];
                for (int i = 0; i < count && dst_ptr < raw_len; ++i) {
        ...
        int copy_len = (src_len < raw_len) ? src_len : raw_len;
...
inline void pcx_decompress_hardcore(const uint8_t* __restrict src, int src_len, uint8_t* __restrict dst, int dst_len) {
    int src_ptr = 128; // 跳过 128 字节 PCX 头
    int dst_ptr = 0;
