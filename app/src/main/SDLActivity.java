...
//        System.setProperty("SDL_HIDAPI_IGNORE_DEVICES", "1");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (USE_FOLDER_SELECT) {
            ...
            // 【终极修复】如果是玩家刚在设置里手动更换目录引发的重启，则强行跳过这次询问
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == FOLDER_PICKER_CODE) {
            mIsPickerActive = false; // 【终极修复】选择完毕，解除物理锁
            ...
            if (isFreshRun) {
                ...
                // 【终极修复】如果是玩家在菜单里点击“重选目录”引发的重启，记录标记，下次启动不要再弹窗烦人了
    ...
    public void openDirectoryPicker() {
        if (mIsPickerActive) return; // 【终极修复】如果选择框已经打开，直接拦截，绝对不允许弹出第二个！
        mIsPickerActive = true;

        if (Build.VERSION.SDK_INT < 24) {
            Log.i(TAG, "检测到旧版安卓(API < 24)，自动指定根目录 IkemenGO 文件夹");
            mIsPickerActive = false; // 解除锁
